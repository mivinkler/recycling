...
class DeliveryExportExcel:
    ...
    def export_response(qs):
        ...
        """
        wb = DeliveryExcelExporter.build_workbook(qs)
...
def delivery_export_excel(request):
    # QuerySet vorbereiten
    qs = DeliveryUnit.objects.select_related(
        "delivery", "delivery__customer", "material"
    ).all()

    return DeliveryExcelExporter.export_response(qs)
